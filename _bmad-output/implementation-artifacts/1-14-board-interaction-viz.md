...
- [ ] **T1 — 棋盘 canvas/SVG 渲染层**（AC1/AC2）
  - [ ] 自由坐标棋盘渲染：英寸坐标 → 屏幕坐标变换；网格背景 + 英寸标尺
  - [ ] 渲染：特工底座圆(阵营色描边) + 朝向三角 + 地形多边形(按 kind 着色) + 目标点(Story 1.16) + 降落区
  - [ ] 缩放/平移：双指(平板)/滚轮+拖(桌面)；手势优先级(UX-OQ-2)先做基础，留 Story 4.1 调优
- [ ] **T2 — 特工拖放 + 实时英寸数**（AC1）
  - [ ] 长按/按下特工 → 拖动 → 松手落定；拖动时浮层显示与起点英寸数（用于回合内合规检查, 如转移 ≤移动值）
  - [ ] 拖动经 store dispatch `MOVE_OPERATIVE`；落定后更新 `OperativePlacement.baseCircle`
  - [ ] 数字输入框备选(x/y/朝向, UX §4.5)；底盘预设 25/32/40mm
- [ ] **T3 — 朝向旋转**（AC1）
  - [ ] 双击特工旋转 45°；朝向三角更新；LOS 头部射线起点随朝向
- [ ] **T4 — LOS 射线 / 射程环 / 控制范围圈渲染**（AC2）
  - [ ] 选中攻击方 + 悬停目标 → 调 geometry 模块(Story 1.8)算 LOS/射程/控制范围
  - [ ] 画 LOS 虚线(受阻段红实线/通畅段绿虚线)、武器射程环(选中武器时)、1" 控制范围圈
  - [ ] 掩护染色：选中特工 → 控制范围内有阻碍地形 → 底圈绿；2" 内有他特工 → 灰
  - [ ] 遮挡：被飞蝇云等遮挡目标 → 半透明虚线轮廓 + 「遮挡」标签
  - [ ] 性能：密集板实时渲染帧率可接受；必要时降级按需显示(UX-OQ-9, 留 Story 4.1)
- [ ] **T5 — 几何 finding 内联翻转（咨询式, D-17/D-24）**（AC3）
  - [ ] 订阅 geometry 产出的 `GeometryFinding{kind,value,confidence,margin,overridden,finalValue}`（架构 §4.4）
  - [ ] AMBIGUOUS 项标「⚠ 可翻转」；点击 1 击翻转假设 → dispatch `OVERRIDE_FINDING` → geometry 翻转 finalValue
  - [ ] 翻转后资格判定(Story 1.10 `TARGET_VALIDATE`)实时重算（资格面板/拦截卡即时更新）
  - [ ] **不弹框、不阻断**；finding 写入 ResolutionLog（接 Story 1.15）
  - [ ] CLEAR 项直接套 value，不显示翻转控件
- [ ] **T6 — 画地形工具（复用 Story 1.12）**（AC4）
  - [ ] 抽取 Story 1.12 的画地形工具栏(矩形/多边形/属性标签)为共享组件；对局中也允许(会话内编辑, D-20)
  - [ ] 与部署期画地形同底层
- [ ] **T7 — 触控与基调兜底**（AC1/AC2）
  - [ ] 拖放/选择命中区 ≥44px；朝向三角可点击
  - [ ] 暗色主题 + 几何线色(红受阻/绿通畅/中性圈)；色盲安全(UX-OQ-10 留视觉阶段)
